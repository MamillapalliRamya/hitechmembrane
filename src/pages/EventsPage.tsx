...
const EventsPage: React.FC = () => {
    ...
    const { translatedText: tNewsHeader } = useTranslateContent("News & Articles");
    ...
    const { translatedText: tFeaturedLabel } = useTranslateContent("Featured");
    const { translatedText: tReadMore } = useTranslateContent("Read More");
    const { translatedText: tUpcomingEvents } = useTranslateContent("Upcoming Events");
    const { translatedText: tPastHighlights } = useTranslateContent("Past Highlights");
    const { translatedText: tCTAMain } = useTranslateContent("Want to meet us at an upcoming event?");
                ...
                <div className="lg:mx-[40px] xl:mx-[60px] 2xl:mx-[140px]">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {filters.map((filter, idx) => (
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-full font-medium transition-colors ${selectedFilter === filter
                                            ? 'bg-[#3E4095] text-white'
                                            : 'bg-white text-gray-700 text-[15.17px] hover:bg-gray-100 border'
                                        }`}
                                >
                                    {useTranslateContent(filter).translatedText}
                                </button>
                            ))}
                        </div>
                            ...
                            >
                                {years.map((year) => (
                                    <option key={year} value={year}>
                                        {useTranslateContent(year).translatedText}
                                    </option>
                                ))}
                            </select>
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {articles.filter(a => !a.featured).map(article => (
                            <div key={article.id} className="bg-white rounded-[18.96px] shadow-md overflow-hidden hover:shadow-xl transition-shadow duration-300">
                                <div className="relative h-48">
                                    <img src={article.image} alt={useTranslateContent(article.title).translatedText} className="w-full h-full object-cover" />
                                </div>
                                <div className="p-6">
                                    <span className="inline-block bg-[#3E4095] text-white px-3 py-1 rounded-full text-xs font-semibold mb-3">
                                        {useTranslateContent(article.category).translatedText}
                                    </span>
                                    <h3 className="text-[20.85px] font-bold text-[#161616] mb-3 line-clamp-2">
                                        {useTranslateContent(article.title).translatedText}
                                    </h3>
                                    <div className="flex items-center text-[#6F6F6F] text-[13.27px] mb-3 font-medium">
                                        <Calendar className="w-4 h-4 mr-2" />
                                        <span>{useTranslateContent(article.date).translatedText}</span>
                                    </div>
                                    <p className="text-gray-600 text-[13.27px] mb-4 line-clamp-3 font-medium">
                                        {useTranslateContent(article.description).translatedText}
                                    </p>
                                    <button className="flex items-center text-indigo-600 font-semibold text-sm hover:text-indigo-700 transition-colors">
                                        {useTranslateContent("Read More").translatedText} <ArrowRight className="w-4 h-4 ml-2" />
                                    </button>
                                </div>
                            </div>
                        ))}
            ...
            <section className="mt-6 lg:mt-9 mb-8 sm:mb-12 lg:mb-16">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 sm:gap-8 mx-[20px]  md:mx-[40px]  xl:px-[70px] 2xl:px-[112px]">
                    {upcomingEvents.map((event) => {
                        const tLocation = useTranslateContent(event.location).translatedText;
                        const tTitle = useTranslateContent(event.title).translatedText;
                        const tDesc = useTranslateContent(event.description).translatedText;

                        return (
                            <div key={event.id} className="bg-white w-full">
                                <div className="relative h-[200px] sm:h-[220px] md:h-[250px] 2xl:h-[300px] overflow-hidden mb-4 sm:mb-6 rounded-2xl sm:rounded-3xl">
                                    <img src={event.image} alt={tTitle} className="w-full h-full object-cover" />
                                    {event.booth && (
                                        <span className="absolute top-3 right-3 sm:top-4 sm:right-4 bg-[#C4F34A] text-[#3E4095] px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-xs sm:text-sm font-bold">
                                            {useTranslateContent(event.booth).translatedText}
                                        </span>
                                    )}
                                </div>
                                <div className="px-2">
        ...
        >
          sales@hitechmembranes.com
                        ...
                        <div className="absolute bottom-4 right-4 sm:bottom-6 sm:right-6 md:bottom-8 md:right-10 w-14 sm:w-16 md:w-20 opacity-90">
                            <img src="/logo-1 (1).png" alt="Hi-Tech Membranes" className="w-full h-auto" />
                        </div>
                    </div>
                </div>
            </div>
